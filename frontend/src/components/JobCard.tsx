...
export const JobCard: React.FC<JobCardProps> = ({ job }) => {
  ...
  return (
    <div className="bg-slate-800/80 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-6 hover:bg-slate-800 transition-all shadow-lg hover:shadow-indigo-500/5 group cursor-pointer" onClick={() => setExpanded(!expanded)}>
      <div className="flex justify-between items-start gap-4">
        <div className="flex-1">
          <h2 className="text-xl font-semibold text-slate-100 group-hover:text-indigo-400 transition-colors line-clamp-2">
            {job.title}
          </h2>
        ...
        <div className={`flex flex-col items-center justify-center min-w-[60px] h-[60px] rounded-xl border ${getScoreColor(job.score)}`}>
          <span className="text-xs font-medium uppercase opacity-70">Score</span>
          <span className="text-xl font-bold leading-tight">{job.score}</span>
        </div>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between">
        <div className="flex items-center gap-2 overflow-hidden">
          ...
          <div className="flex gap-2 overflow-x-auto no-scrollbar mask-fade-right pb-1">
            {job.skills.slice(0, 5).map((skill, idx) => (
              <span key={idx} className="px-2.5 py-1 rounded-full bg-slate-700/50 text-slate-300 text-xs whitespace-nowrap">
                {skill}
              </span>
            ))}
            {job.skills.length > 5 && (
              <span className="px-2.5 py-1 rounded-full bg-slate-700/30 text-slate-400 text-xs whitespace-nowrap">
                +{job.skills.length - 5}
              </span>
            )}
          </div>
        ...
        <a 
          ...
          className="shrink-0 flex items-center gap-1.5 px-4 py-2 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 rounded-lg transition-colors text-sm font-medium"
        >
          View Job
